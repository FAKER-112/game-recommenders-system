...
This test suite provides comprehensive unit testing for the PredictionPipeline class
in `src/pipeline/predict_pipeline.py`, which handles model loading and inference for
all three recommendation system models: Autoencoder, Matrix Factorization (MF), and TFRS.

Test Coverage:
-------------

**Initialization Tests (2 tests)**
- test_init_success: Validates proper initialization with config loading
- test_init_failure: Tests CustomException handling on initialization errors

**Model Loading Tests (5 tests)**
- test_load_model_mf: Tests Matrix Factorization model loading from H5 file
- test_load_model_tfrs: Tests TFRS retrieval and item index loading
- test_load_model_tfrs_file_not_found: Tests error handling when TFRS files are missing
- test_load_model_invalid_name: Tests exception handling for invalid model names

**Artifact Loading Tests (3 tests)**
- test_load_artifacts_mf: Tests loading MF encoders (user_encoder, item_encoder)
- test_load_artifacts_tfrs: Tests loading TFRS candidates DataFrame
- test_load_artifacts_autoencoder: Tests loading Autoencoder artifacts (X, indices, itemlist)

**Recommendation Tests (6 tests)**
- test_recommend_dispatches_to_tfrs: Tests routing to TFRS recommendation method
- test_recommend_dispatches_to_mf: Tests routing to MF recommendation method
- test_recommend_not_implemented: Tests fallback for Autoencoder (not implemented)
- test_recommend_tfrs: Tests TFRS recommendation logic with retrieval index
- test_recommend_mf: Tests MF recommendation logic with encoder and model
- test_recommend_mf_user_not_found: Tests handling of unknown users in MF

**Similar Items Tests (3 tests)**
- test_get_similar_items_dispatches_to_tfrs: Tests routing to TFRS similar items method
- test_get_similar_items_dispatches_to_autoencoder: Tests routing to Autoencoder method
- test_get_similar_items_tfrs: Tests TFRS similar items logic with item index

Testing Strategy:
----------------
All tests use extensive mocking to ensure:
- Fast execution (no actual model loading or inference)
- Isolation from file system dependencies
- Consistent and reproducible test results

Common mocks set up in setUp():
- load_config: Returns mock configuration
- logger: Prevents actual logging
- os.path.exists: Returns True by default
- Model-specific mocks added per test (tf.keras.models.load_model, pickle.load, etc.)
...
Run specific test:
    python -m pytest tests/test_predict_pipeline.py::TestPredictionPipeline::test_recommend_mf -v

Run with unittest:
    python -m unittest tests.test_predict_pipeline
"""
...
class TestPredictionPipeline(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.load_config_patcher = patch(
            "src.pipeline.predict_pipeline.load_config", return_value=self.mock_config
        )
    ...
    @patch(
        "src.pipeline.predict_pipeline.load_config",
    ...
    def test_load_model_mf(self, mock_load_keras_model):
        ...
        self.mock_logger.info.assert_called_with(
            "Loaded MF model from artifacts/models/mf_model.h5"
        )
    ...
    def test_load_model_tfrs(self, mock_tf_load):
        ...
        self.mock_logger.info.assert_any_call(
            "Loaded TFRS retrieval index from artifacts/models/tfrs_retrieval_index"
        )
        self.mock_logger.info.assert_any_call(
            "Loaded TFRS item index from artifacts/models/tfrs_item_index"
        )
    ...
    def test_load_artifacts_tfrs(self, mock_read_csv):
        ...
        self.mock_logger.info.assert_called_with(
            f"Loaded {len(mock_df)} candidates from {candidates_path}"
        )
    ...
    @patch("json.load")
    @patch("builtins.open", new_callable=mock_open)
    def test_load_artifacts_autoencoder(
        self, mock_file, mock_json_load, mock_read_csv, mock_np_load
    ):
        ...
        mock_np_load.assert_called_once_with(
            os.path.join("artifacts/context", "autoencoder_X.npy")
        )
        mock_read_csv.assert_called_once_with(
            os.path.join("artifacts/context", "autoencoder_indices.csv")
        )
        mock_file.assert_called_with(
            os.path.join("artifacts/context", "autoencoder_itemlist.json"), "r"
        )
    ...
    def test_recommend_dispatches_to_tfrs(self, mock_recommend_tfrs):
        ...
        with patch(
            "src.pipeline.predict_pipeline.PredictionPipeline._load_model"
        ), patch("src.pipeline.predict_pipeline.PredictionPipeline._load_artifacts"):
            pipeline = PredictionPipeline(model_name="tfrs")
    ...
    def test_recommend_not_implemented(self):
        ...
        self.mock_logger.warning.assert_called_with(
            "Recommend not implemented for autoencoder"
        )
    ...
    def test_recommend_mf_user_not_found(self):
        ...
        self.mock_logger.warning.assert_called_with(
            "User user1 not found in training data."
        )
    ...
    @patch(
        "src.pipeline.predict_pipeline.PredictionPipeline._get_similar_items_autoencoder"
    )
    ...
    def test_get_similar_items_tfrs(self):
        ...
        similar_titles_tensor = tf.constant(
            [["item1", "sim1", "sim2"]], dtype=tf.string
        )
