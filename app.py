...
def health():
    ...
    return jsonify({
        ...
        'data_loaded': {
            'games': len(GAMES_DATA) > 0,
            'users': len(USER_IDS) > 0
        },
        'counts': {
...
def recommend_user():
    ...
    logger.info(f"Generating {n_rec} recommendations for user {user_id} using {model_name}")
    ...
    for game_name in recommendations:
        for game in GAMES_DATA:
            if (game.get('title', '').lower() == game_name.lower() or 
                game.get('app_name', '').lower() == game_name.lower()):
...
def recommend_item():
    ...
    if not isinstance(k, int) or k < 1 or k > 100:
        return jsonify({
            'error': 'k must be an integer between 1 and 100',
            'status': 'error'
        }), 400
