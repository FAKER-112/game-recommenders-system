        ...
        try:
            ...
            with mlflow.start_run(run_name="TFRS_Retrieval"):
                ...
                context_data = {
                    ...
                    "user_vocab_size": data_dict["user_ids_vocabulary"].vocabulary_size(),
                    "item_vocab_size": data_dict["item_titles_vocabulary"].vocabulary_size(),
                ...
                with open(text_vocab_path, "wb") as f:
                    pickle.dump({"config": text_vocab_config, "weights": text_weights}, f)
                
                # Save context metadata
                context_metadata_path = os.path.join(self.context_dir, "tfrs_metadata.json")
                ...
                index_user = tfrs.layers.factorized_top_k.BruteForce(tfrs_model.user_model)
                index_user.index_from_dataset(
                    data_dict["items"].batch(128).map(
                        lambda x: (x["item_name"], tfrs_model.item_model(x))
                    )
                )
                ...
                self.logger.info(f"User-to-item retrieval index saved to {index_user_path}")
                
                # 4. Create and save ITEM-TO-ITEM retrieval index for similar items
                item_to_item_index = tfrs.layers.factorized_top_k.BruteForce(tfrs_model.item_model)
                ...
                self.logger.info(f"Item-to-item retrieval index saved to {item_index_path}")
                ...
                self.logger.info("TFRS model training and saving completed successfully")
